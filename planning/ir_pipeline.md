...
| `body(def)` | `hir/body.rs` | Lower + name-resolve the body into the per-def arenas; split `var` decls from equations; record declared types on `let`/`var` locals. Diagnoses unresolved names/types, overflowing literals, and direction prefixes that disagree with their connector. |
...
| `const_eval` (helper, `hir/const_eval.rs`) | `hir/const_eval.rs` | Demand-driven interpreter over body HIR (`planning/const_eval.md`): per-local thunks (let / driving equation / call out-connection), memoized with cycle markers; if/else, records, prelude arithmetic on `integer` (i128). Not a salsa query — called from `infer`/backend, deterministic from `body()` inputs. Note: this reaches *callee bodies* (the one deliberate exception to the sig-only firewall, as in rustc CTFE). |
| `check_drivers(def)` | `hir/check.rs` | Per-leaf drive paths: every `var` is driven, with no two drives overlapping (one path a prefix of the other); disjoint per-field wiring is legal. |
| `directions(def)` | `hir/check.rs` | Connection operators agree with port-field / param direction (`=` in, `=>` out). |
| `sv_module(def)` | `backend/lower.rs` | Per-def lowering to one SV module: register recognition (`.reg`), block/if/when statement-forming, method-call rewriting, out-arg desugaring, aggregate flattening (structs/ports → per-field leaves, domain stamping, direction-aware equation splitting), type-generic monomorphisation at call sites. Widths ground through `const_eval` at the type chokepoints; `integer`-typed locals/ports and const-only fns are elided (compile-time only). Width residuals emit as `initial assert`. |
