...
- Resolve inferable parameters (`#clk`) beyond what was already explicit in the source.
...
- `reg : fn{#clk: Clock}(self @clk, rst: Reset @clk, reset_val: uint(N)) -> uint(N) @clk`
...
- Resolve inferable arguments through cyclic `var` equations, or even through anchored `Reset @clk` arguments. All `#clk` slots that the user omitted become `Inferable` and are handed to type inference.
