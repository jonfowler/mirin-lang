...
Landed and removed from here: `simple_port.mrn` and `parameterized_port.mrn` (now `working/simple_port.mrn`, `working/parameterized_port.mrn`), `parameterized_struct.mrn` (now `working/parametric_struct.mrn` and `working/parametric_struct_extended.mrn`), the `multi_module.mrn` stub (its port-impl sketch is covered by `impl_examples.mrn`), and `df_example.mrn` (now `working/df_example.mrn` + the payload-polymorphic `working/df_example_poly.mrn` + `tests/rtl/test_df_example.py` — a valid/ready forward register; landed with the boolean operators `!`/`||`/`&&`).
