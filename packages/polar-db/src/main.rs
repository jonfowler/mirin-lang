//! `polar-db` CLI — compile a `.plr` file (and the `mod foo;` files it pulls in)
//! to SystemVerilog through the query stack. The batch driver fills the
//! [`Vfs`] from disk once, builds the crate's [`SourceRoot`], reports any
//! front-end diagnostics, and writes `verilog(crate)` to `<out>/<stem>.sv`.
//!
//! This is the Q5e "CLI swap": polar-db is now the primary compiler. The old
//! `polar-compiler` stays in the workspace as the parity oracle until
//! parametric monomorphisation (Q5-mono) lands.
...
fn main() {
    let program = env::args_os()
        ...
        .unwrap_or_else(|| "polar-db".to_owned());
