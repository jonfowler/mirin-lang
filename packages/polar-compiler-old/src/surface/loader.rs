//! Multi-file loader — turns a crate root `.plr` file plus its `mod foo;`
//! declarations into one combined [`SourceFile`].
//!
//! File-based modules (`planning/modules.md` §4.2): `mod foo;` inside
//! `dir/X.plr` loads from `dir/foo.plr`, and that module's own children live
//! under `dir/foo/`. A `.plr` file is part of the crate only if some ancestor
...
/// Supplies `.plr` source text for a path. The filesystem implementation
    ...
    fn fill_modules(&mut self, items: &mut [Item], dir: &Path) -> Result<(), LoadError> {
        for item in items {
            ...
            match &mut m.body {
                ...
                ModuleBody::File => {
                    let file_path = dir.join(format!("{}.plr", m.name.text));
...
mod tests {
    ...
    fn loads_file_module_body() {
        let crate_ = load(
            &[
                ("/crate/main.plr", "mod util;\n"),
                ("/crate/util.plr", "fn helper(a: uint(8)) { let r = a; }\n"),
            ],
            "/crate/main.plr",
        );
    ...
    fn node_ids_are_unique_across_files() {
        let crate_ = load(
            &[
                ("/c/main.plr", "mod a;\nmod b;\n"),
                ("/c/a.plr", "fn fa(x: uint(8)) { let r = x; }\n"),
                ("/c/b.plr", "fn fb(x: uint(8)) { let r = x; }\n"),
            ],
            "/c/main.plr",
        );
    ...
    fn nested_directory_layout() {
        // main.plr → mod util; (util.plr) → mod cfg; (util/cfg.plr)
        let crate_ = load(
            &[
                ("/c/main.plr", "mod util;\n"),
                ("/c/util.plr", "mod cfg;\n"),
                ("/c/util/cfg.plr", "fn parse(x: uint(8)) { let r = x; }\n"),
            ],
            "/c/main.plr",
        );
        let r = resolve_file(&crate_.file);
    ...
    fn missing_module_file_is_load_error() {
        let mut provider = MapProvider::new();
        provider.insert("/c/main.plr", "mod gone;\n");
        let err = load_crate(&provider, Path::new("/c/main.plr")).unwrap_err();
