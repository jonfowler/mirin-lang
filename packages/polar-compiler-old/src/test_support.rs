...
//! test time via `std::fs::read_dir`, so adding a new `.plr` file under
...
/// `(name, source)` for every `.plr` file under `examples/working/`, sorted by
...
/// `(name, source)` for every `.plr` file under `examples/fail-expected/`,
/// sorted by name.
#[allow(dead_code)]
...
fn read_plr_dir(dir: &Path) -> Vec<(String, String)> {
    ...
    for entry in entries {
        ...
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()) != Some("plr") {
            continue;
        }
        let name = path
            .file_stem()
