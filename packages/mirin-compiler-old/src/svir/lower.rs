...
/// Build the SV module name for a Mirin function. Methods are qualified by
...
fn lower_fn(func: &HirFn, defs: &BackendDefs<'_>, residuals: &[FnResidual]) -> SvModule {
    ...
    // Disambiguate Mirin identifier names for the SV-side namespace. Mirin
...
/// Build a `LocalId → unique SV identifier` map for a function. Mirin
...
mod tests {
    ...
    fn lowers_accumulator() {
        let sv = lower(include_str!("../../../../examples/working/accumulator.mrn"));
    ...
    fn lowers_counter_with_parameter() {
        let sv = lower(include_str!("../../../../examples/working/counter.mrn"));
        let m = &sv.modules[0];
