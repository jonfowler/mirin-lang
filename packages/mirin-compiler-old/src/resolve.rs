...
/// `mirin` build tool and multi-crate loading land (S3+), this is the only
...
/// The two name namespaces (Rust has three; Mirin has no macros). A module's
...
/// `mirin-db` crate's `ids::Namespace`) splits **{Module, Item}** instead: a type
...
/// model forward; the correct one lives in `mirin-db`.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub enum Namespace {
    Type,
    Value,
...
pub enum ResolveErrorKind {
    ...
    /// Mirin requires every nominal type to declare its term-level constructor
    /// name explicitly, the way `struct Foo<T> { … }` in Rust pairs with the
    /// `Foo { … }` constructor — except Mirin uses a distinct constructor name
...
pub fn resolve_file(file: &SourceFile) -> ResolveResult {
    ...
    // (Mirin has no macros, so this needs no expansion/fixpoint).
...
impl Ctx {
    ...
    /// Mirin has no macros, so this is the only fixpoint resolution needs.
    ...
    fn resolve_expr(&mut self, expr: &Expression) {
        match expr {
            ...
            Expression::Block(b) => {
                ...
                // enclosing function's var namespace (Mirin `var`s are
...
mod tests {
    ...
    fn resolves_example_file() {
        let source = include_str!("../../../examples/working/mult_add.mrn");
    ...
    fn name_resolution_fail_undefined_name() {
        let source = include_str!("../../../examples/fail-expected/undefined-name.mrn");
        let r = resolve_file_source(source);
        assert_eq!(r.errors.len(), 1, "errors: {:?}", r.errors);
        assert!(
    ...
    fn name_resolution_fail_duplicate_def() {
        let source = include_str!("../../../examples/fail-expected/duplicate-def.mrn");
        let r = resolve_file_source(source);
        assert_eq!(r.errors.len(), 1, "errors: {:?}", r.errors);
        assert!(
    ...
    fn name_resolution_fail_duplicate_var() {
        let source = include_str!("../../../examples/fail-expected/duplicate-var.mrn");
        let r = resolve_file_source(source);
        assert_eq!(r.errors.len(), 1, "errors: {:?}", r.errors);
        assert!(
    ...
    fn name_resolution_fail_var_after_let() {
        let source = include_str!("../../../examples/fail-expected/var-after-let.mrn");
        let r = resolve_file_source(source);
        assert_eq!(r.errors.len(), 1, "errors: {:?}", r.errors);
        assert!(
