...
use mirin_compiler_old::hirt::typeck;
use mirin_compiler_old::{
...
fn main() {
    ...
    let loaded = match load_crate_from_fs(&args.input) {
        ...
        Err(e) => {
            ...
            process::exit(match e {
                mirin_compiler_old::LoadError::Read { .. } => 2,
                _ => 1,
            });
        }
    };
    ...
    let mono = mirin_compiler_old::hirtl::monomorphise::monomorphise(
        hir,
        tc.expr_types,
        tc.local_types,
    ...
    let block_lowered =
        mirin_compiler_old::lower_block_expressions(&hir, &mono_expr_types, &mono_local_types);
    ...
    let hir = mirin_compiler_old::lower_method_calls(&hir, &result, &mono_method_resolutions);
    ...
    let hir = match mirin_compiler_old::desugar_user_calls(&hir) {
        Ok(h) => h,
        Err(errors) => {
            for (i, err) in errors.iter().enumerate() {
                if i > 0 {
    ...
    let flat = match flatten_aggregates(&hir, &result, &mono_expr_types, &local_types) {
        ...
        Err(errors) => {
            let mut rendered = String::new();
            mirin_compiler_old::render_flatten_errors(
                &errors,
                &source,
                Some(&args.input),
                &mut rendered,
