...
unsafe extern "C" {
    fn tree_sitter_mirin() -> *const ();
}

const LANGUAGE_FN: LanguageFn = unsafe { LanguageFn::from_raw(tree_sitter_mirin) };
...
mod tests {
    ...
    fn parses_component_examples_into_cst() {
        for source in [
            include_str!("../../../../../examples/working/add_constant.mrn"),
            include_str!("../../../../../examples/working/mult_add.mrn"),
            include_str!("../../../../../examples/working/counter.mrn"),
            include_str!("../../../../../examples/working/shift_register.mrn"),
        ] {
    ...
    fn root_span_covers_full_source() {
        let source = include_str!("../../../../../examples/working/add_constant.mrn");
        let cst = parse_source(source).unwrap();
    ...
    fn reports_missing_named_section_closer_once() {
        let source = include_str!("../../../../../examples/fail-expected/missing-parenthesis.mrn");
    ...
    fn rejects_all_failure_examples() {
        for source in [
            include_str!("../../../../../examples/fail-expected/missing-parenthesis.mrn"),
            include_str!("../../../../../examples/fail-expected/missing-semicolon.mrn"),
            include_str!("../../../../../examples/fail-expected/missing-struct-colon.mrn"),
            include_str!("../../../../../examples/fail-expected/missing-port-comma.mrn"),
        ] {
            let error = parse_source(source).unwrap_err();
            let ParseError::Syntax(diagnostics) = error else {
                panic!("expected syntax diagnostics");
    ...
    fn preserves_cst_when_reporting_diagnostics() {
        let source = include_str!("../../../../../examples/fail-expected/missing-semicolon.mrn");
