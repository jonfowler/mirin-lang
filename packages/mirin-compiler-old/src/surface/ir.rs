...
/// file-backed — the loader reads `foo.mrn` and replaces the body with the
...
pub enum ModuleBody {
    ...
    /// resolved `File` body into `Inline` once `foo.mrn` is loaded, so by the
    /// time resolution runs every module is `Inline`.
    Inline(Vec<Item>),
    /// `mod foo;` — body lives in `foo.mrn`, not yet loaded.
    File,
}
...
pub enum Expression {
    ...
    /// in parens.
    If(Box<IfExpression>),
    /// `when EVENT { … }` — Mirin's primitive for registered state. The
...
pub struct TypeExpression {
    ...
    /// Ordered positionally within the section, like Mirin's other named
