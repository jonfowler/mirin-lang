...
fn bin() -> Command {
    Command::new(env!("CARGO_BIN_EXE_mirin-compiler-old"))
}
...
fn nonexistent_file_exits_2() {
    let status = bin().arg("no-such-file.mrn").status().unwrap();
    assert_eq!(status.code(), Some(2));
}
...
fn emit_cst_prints_to_stdout() {
    let output = bin()
        .args(["--emit", "cst"])
        .arg(examples().join("add_constant.mrn"))
        .output()
        .unwrap();
    assert!(
...
fn default_emits_sv_to_out_dir() {
    ...
    let output = bin()
        ...
        .arg(examples().join("accumulator.mrn"))
        .output()
        .unwrap();
    assert!(
        output.status.success(),
...
fn success_produces_no_stderr() {
    ...
    for name in &["counter", "mult_add", "accumulator", "pipeline"] {
        let output = bin()
            .args(["--out".as_ref(), out_dir.as_os_str()])
            .arg(examples().join(format!("{name}.mrn")))
            .output()
            .unwrap();
        assert!(
            output.status.success(),
...
fn reserved_word_exits_1_with_message() {
    ...
    let output = bin()
        .args(["--out".as_ref(), out_dir.as_os_str()])
        .arg(fail_examples().join("sv-reserved-word.mrn"))
        .output()
        .unwrap();
...
fn parse_error_exits_1_with_message_on_stderr() {
    let output = bin()
        .arg(fail_examples().join("missing-semicolon.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
    ...
    assert!(
        stderr.contains("missing-semicolon.mrn"),
...
fn resolve_undefined_name_exits_1() {
    let status = bin()
        .arg(fail_examples().join("undefined-name.mrn"))
        .status()
        .unwrap();
...
fn resolve_undefined_name_message() {
    ...
    assert!(
        stderr.contains("undefined-name.mrn"),
        "expected filename in output, got: {stderr}"
    );
    assert!(output.stdout.is_empty(), "expected nothing on stdout");
}
...
fn resolve_duplicate_def_message() {
    let output = bin()
        .arg(fail_examples().join("duplicate-def.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn resolve_duplicate_var_message() {
    let output = bin()
        .arg(fail_examples().join("duplicate-var.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn resolve_var_after_let_message() {
    let output = bin()
        .arg(fail_examples().join("var-after-let.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn direction_unknown_named_arg_message() {
    let output = bin()
        .arg(fail_examples().join("unknown-named-arg.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
...
fn direction_source_arrow_on_fn_param_message() {
    let output = bin()
        .arg(fail_examples().join("source-arrow-on-fn-param.mrn"))
        .output()
        .unwrap();
    assert_eq!(output.status.code(), Some(1));
    let stderr = String::from_utf8_lossy(&output.stderr);
