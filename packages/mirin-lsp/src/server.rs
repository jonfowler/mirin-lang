...
impl Backend {
    ...
    /// nodes) always; semantic (the `mirin-db` query stack) only once the file
...
impl LanguageServer for Backend {
    ...
    async fn initialized(&self, _: InitializedParams) {
        ...
        info!(encoding = enc, "mirin-lsp initialized");
        // Also surface it in the editor's LSP log (proves the client channel).
        self.client
            .log_message(
                MessageType::INFO,
                format!("mirin-lsp initialized (position encoding: {enc})"),
            )
            .await;
    }
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        // re-parse. `mirin-fmt` refuses files with syntax errors; tell the user
        // why but return "no edits" rather than wiping a buffer the user is
        // mid-typing in.
        let source = doc.rope.to_string();
        let formatted = match mirin_fmt::format_tree(&source, &doc.tree) {
            Ok(text) => text,
            Err(e) => {
                self.client
                    .show_message(MessageType::WARNING, format!("mirin-fmt: {e}"))
                    .await;
                return Ok(None);
            }
        };
